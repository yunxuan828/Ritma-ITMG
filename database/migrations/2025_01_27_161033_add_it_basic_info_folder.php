...
return new class extends Migration
{
    public function up()
    {
        ...
        DB::table('i_t_folders')->insert([
            'name' => 'IT Basic Info',
            'description' => 'Basic IT information and documentation',
            'created_by' => DB::raw('(SELECT id FROM users ORDER BY id LIMIT 1)'), // Gets the first user ID if exists
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        ...
        // Move any existing files without a folder to IT Basic Info folder
        if ($itBasicInfoId) {
            DB::table('i_t_file_sharings')
                ->whereNull('folder_id')
                ->update(['folder_id' => $itBasicInfoId]);
    ...
    public function down()
    {
        // Simply delete the IT Basic Info folder
        // Files will remain in the database with null folder_id
        DB::table('i_t_folders')
            ->where('name', 'IT Basic Info')
            ->delete();
    }
